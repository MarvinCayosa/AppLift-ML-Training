"""
AppLift ML Training - SVM Classifier (REALISTIC VERSION)
=========================================================
A comprehensive classification pipeline for exercise execution quality using Support Vector Machines.
...
3. SVM-SPECIFIC OPTIMIZATIONS:
   ...
   - Efficient scaling (critical for SVM)
...
# SVM VERSION: Use separate output directory for SVM model results
OUTPUT_DIR = PROJECT_ROOT / 'outputs' / 'svm'
MODELS_DIR = OUTPUT_DIR / 'models'
MODELS_DIR.mkdir(parents=True, exist_ok=True)

COMPARISON_DIR = OUTPUT_DIR / 'comparison_with_others'
...
def get_default_svm_params(kernel='rbf', class_weight='balanced'):
    """
    Default SVM parameters optimized for exercise quality classification.
    
    Parameters:
    - kernel: Kernel type ('rbf', 'linear', 'poly', 'sigmoid')
    - class_weight: Weight for class imbalance ('balanced' or None)
    
    Returns:
    - Dictionary of SVM parameters
    """
    params = {
        'C': 1.0,
        'kernel': kernel,
        ...
        'random_state': 42,
        'max_iter': -1,  # No limit
        'cache_size': 1000  # MB for kernel cache
    }
...
def create_optimized_svm_model(best_params=None, kernel='rbf', class_weight='balanced'):
    """
    Create SVM model with optimized hyperparameters or defaults.
    ...
    """
    if best_params:
        print(f"\n[SVM] Creating optimized SVM model...")
        ...
        print(f"\n[SVM] Creating default SVM model with {kernel} kernel...")
        model_params = get_default_svm_params(kernel=kernel, class_weight=class_weight)
...
def get_hyperparameter_grid_svm(kernel='rbf', use_imbalance_strategy=True):
    """
    Define hyperparameter grid for SVM Grid Search.
    
    Parameters:
    - kernel: Kernel type to optimize for
    - use_imbalance_strategy: Whether to include class_weight options
...
def get_hyperparameter_distributions_svm(kernel='rbf', use_imbalance_strategy=True):
    """
    Define hyperparameter distributions for SVM Random Search.
...
def perform_grid_search_svm(X_train, y_train, kernel='rbf', cv_folds=5, n_jobs=-1, verbose=1, use_imbalance_strategy=True):
    """
    ...
    - use_imbalance_strategy: Whether to search class_weight
    
    Returns:
    - best_params: Dictionary of best hyperparameters
    - grid_search: GridSearchCV object
    """
    print(f"\n[GRID] Performing Grid Search for SVM hyperparameters ({kernel} kernel)...")
    print(f"  CV Folds: {cv_folds} | Parallel Jobs: {n_jobs}")
    
    param_grid = get_hyperparameter_grid_svm(kernel=kernel, use_imbalance_strategy=use_imbalance_strategy)
    
    print(f"  Parameter combinations: {np.prod([len(v) if isinstance(v, list) else 1 for v in param_grid.values()]):,}")
    print(f"  Total fits: {np.prod([len(v) if isinstance(v, list) else 1 for v in param_grid.values()]) * cv_folds:,}")
...
def perform_random_search_svm(X_train, y_train, kernel='rbf', n_iter=50, cv_folds=5, n_jobs=-1, verbose=1, use_imbalance_strategy=True):
    """
    Perform Random Search for SVM hyperparameter optimization.
    ...
    """
    print(f"\n[RANDOM] Performing Random Search for SVM hyperparameters ({kernel} kernel)...")
    print(f"  Iterations: {n_iter} | CV Folds: {cv_folds} | Parallel Jobs: {n_jobs}")
    
    param_distributions = get_hyperparameter_distributions_svm(kernel=kernel, use_imbalance_strategy=use_imbalance_strategy)
...
def train_svm(
    ...
    """
    Train an SVM classifier with optional optimized hyperparameters.
    ...
    print("  [CRITICAL] Scaling features (required for SVM)...")
    ...
    if best_params:
        ...
        print(f"  Using default hyperparameters with {kernel} kernel")
    
    model = create_optimized_svm_model(
    ...
    print(f"  * Support Vectors: {model.n_support_.sum()} ({model.n_support_.sum()/len(X_train_scaled):.1%} of training data)")
...
def perform_cross_validation_svm(X_train, y_train, n_splits=5, best_params=None, kernel='rbf', class_weight='balanced'):
    """
    ...
    if gap > 0.15:
        ...
        print("     Consider: reducing C, using linear kernel, or adding more regularization")
    elif cv_results['balanced_accuracy_mean'] < 0.6:
        print("  [WARNING] Possible UNDERFITTING detected (CV balanced accuracy < 0.60)")
        print("     Consider: increasing C, trying RBF kernel, or adding more features")
    else:
...
def perform_cross_validation_with_smote_svm(X_train, y_train, n_splits=5, best_params=None, 
                                            kernel='rbf', class_weight='balanced', quality_names=None):
    """
    ...
    if not SMOTE_AVAILABLE:
        ...
        return perform_cross_validation_svm(X_train, y_train, n_splits, best_params, kernel, class_weight)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv.split(X_train, y_train)):
        ...
        model = create_optimized_svm_model(best_params=best_params, kernel=kernel, class_weight=class_weight)
...
def create_model_visualizations_svm(y_test, results, cv_results, output_folder, exercise_code=None, df=None):
    """
    Create comprehensive visualizations for SVM model performance.
    ...
    n_sv = results['support_vectors_per_class']
    plt.bar(range(len(n_sv)), n_sv, color='steelblue')
    plt.xticks(range(len(n_sv)), class_names, rotation=45, ha='right')
    plt.xlabel('Class')
    plt.ylabel('Number of Support Vectors')
    plt.title('Support Vectors per Class', fontsize=14, fontweight='bold')
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig(viz_folder / 'support_vectors.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if cv_results:
        ...
        plt.suptitle('SVM Cross-Validation Performance', fontsize=14, fontweight='bold')
        plt.tight_layout()
...
def export_model_svm(model, scaler, feature_names, results, cv_results, output_folder, 
                     exercise_code=None, df=None, imbalance_config=None, reduction_summary=None):
    """
    Export trained SVM model and comprehensive metadata.
    ...
    model_file = output_path / f'svm_model_{timestamp}.pkl'
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        # Executive Summary
        f.write("=" * 80 + "\n")
        f.write("SVM CLASSIFIER - COMPREHENSIVE EVALUATION REPORT\n")
        f.write("=" * 80 + "\n\n")
        
        f.write("EXECUTIVE SUMMARY\n")
        f.write("-" * 80 + "\n")
        f.write(f"This report presents a comprehensive evaluation of a Support Vector Machine classifier\n")
        ...
        f.write(f"The model uses {model.n_support_.sum()} support vectors ({model.n_support_.sum()/len(results.get('y_test', []))*100:.1f}% of training data).\n")
        f.write("\n")
        ...
        if imbalance_config:
            ...
            f.write(f"Class Imbalance Strategy: {imbalance_config.get('strategy', 'class_weight=balanced')}\n")
        f.write("\n")
        ...
        if model.kernel == 'poly':
            ...
            f.write(f"Coef0:                   {model.coef0}\n")
        f.write(f"Class Weight:            {model.class_weight}\n")
        f.write(f"Probability:             {model.probability}\n")
        f.write(f"Cache Size:              {model.cache_size} MB\n")
        f.write(f"Random State:            {model.random_state}\n")
        f.write(f"Support Vectors:         {model.n_support_.sum()} total\n")
        for i, n_sv in enumerate(model.n_support_):
            class_name = quality_names.get(i, f'Class {i}')
            f.write(f"  - {class_name}: {n_sv} support vectors\n")
        f.write("\n")
        ...
        f.write("SVM-SPECIFIC ANALYSIS\n")
        f.write("-" * 80 + "\n")
        f.write("Support Vector Machine Characteristics:\n")
        f.write(f"  - Kernel function: {model.kernel}\n")
        f.write(f"  - Decision boundary: Maximum margin hyperplane\n")
        f.write(f"  - Total support vectors: {model.n_support_.sum()}\n")
        f.write(f"  - Support vector ratio: {model.n_support_.sum()/len(results.get('y_test', []))*100:.1f}% of training data\n")
        f.write(f"  - Regularization (C): {model.C} (lower = more regularization)\n")
        f.write(f"  - Gamma: {model.gamma} (kernel coefficient)\n")
        
        f.write(f"\nSupport Vectors per Class:\n")
        for i, n_sv in enumerate(model.n_support_):
            class_name = quality_names.get(i, f'Class {i}')
            percentage = (n_sv / model.n_support_.sum()) * 100
            f.write(f"  - {class_name}: {n_sv} vectors ({percentage:.1f}%)\n")
        
        f.write(f"\nKernel Analysis:\n")
        if model.kernel == 'rbf':
            f.write(f"  - RBF (Radial Basis Function) kernel selected\n")
            f.write(f"  - Good for non-linear decision boundaries\n")
            f.write(f"  - Gamma={model.gamma} controls influence of single training example\n")
        elif model.kernel == 'linear':
            f.write(f"  - Linear kernel selected\n")
            f.write(f"  - Suitable for linearly separable data\n")
            f.write(f"  - Faster training and prediction\n")
        elif model.kernel == 'poly':
            f.write(f"  - Polynomial kernel selected (degree={model.degree})\n")
            f.write(f"  - Models polynomial decision boundaries\n")
            f.write(f"  - Coef0={model.coef0} affects polynomial shape\n")
        ...
        f.write(f"- Generalization Gap:    {cv_results.get('generalization_gap', 0):.4f}\n")
        f.write(f"- Support Vector Ratio:  {model.n_support_.sum()/len(results.get('y_test', []))*100:.1f}%\n")
        ...
        # Support vector analysis
        sv_ratio = model.n_support_.sum()/len(results.get('y_test', []))*100
        if sv_ratio < 20:
            f.write(f"- EFFICIENT: Low support vector ratio ({sv_ratio:.1f}%) indicates good generalization\n")
        elif sv_ratio < 50:
            f.write(f"- MODERATE: Medium support vector ratio ({sv_ratio:.1f}%)\n")
        else:
            f.write(f"- COMPLEX: High support vector ratio ({sv_ratio:.1f}%) may indicate overfitting\n")
        ...
        f.write("4. Consider ensemble with RF/XGBoost for critical applications\n")
        f.write("5. Monitor for concept drift in production environment\n")
        
        if sv_ratio > 50:
            f.write("6. High support vector ratio - consider reducing C or using linear kernel\n")
        ...
        f.write(f"- Model Complexity:      {model.n_support_.sum()} support vectors\n")
        f.write(f"- Data Preprocessing:    StandardScaler (CRITICAL for SVM)\n")
...
def run_classification_pipeline_svm():
    """
    Main function to run the complete SVM classification pipeline.
    """
    print("=" * 80)
    print("  SVM EXERCISE QUALITY CLASSIFICATION PIPELINE")
    print("=" * 80)
    print("\nThis pipeline trains a Support Vector Machine classifier for exercise quality assessment.")
    ...
    print("\n[STEP 9] Dimensionality Reduction")
    print("-" * 80)
    print("[NOTE] Dimensionality reduction is HIGHLY RECOMMENDED for SVM to improve training speed")
    ...
    # Step 10: Kernel selection
    print("\n[STEP 10] Kernel Selection")
    print("-" * 80)
    print("\nChoose SVM kernel:")
    print("1. [RBF] Radial Basis Function (default, good for non-linear data)")
    print("2. [LINEAR] Linear kernel (faster, good for linearly separable data)")
    print("3. [POLY] Polynomial kernel (flexible, but slower)")
    print("4. [ALL] Try all kernels during hyperparameter search")
    ...
    kernel_map = {'1': 'rbf', '2': 'linear', '3': 'poly', '4': 'all'}
    kernel = kernel_map[choice]
    ...
    print("2. [RANDOM] Random Search (50 iterations, ~10-15 min)")
    print("3. [GRID] Grid Search (comprehensive, ~20-40 min)")
    ...
    best_params = None
    if choice == '2':
        best_params, _ = perform_random_search_svm(
            X_train_reduced, y_train,
            kernel=kernel, n_iter=50, cv_folds=5, n_jobs=-1, verbose=1,
            use_imbalance_strategy=imbalance_config['use_imbalance_strategy']
        )
    elif choice == '3':
        best_params, _ = perform_grid_search_svm(
            X_train_reduced, y_train,
            kernel=kernel, cv_folds=5, n_jobs=-1, verbose=1,
            use_imbalance_strategy=imbalance_config['use_imbalance_strategy']
        )
    ...
    if imbalance_config['use_smote']:
        cv_results = perform_cross_validation_with_smote_svm(
            X_train_reduced, y_train,
            n_splits=5, best_params=best_params,
            kernel=kernel if not best_params else best_params.get('kernel', kernel),
            class_weight='balanced' if imbalance_config['use_imbalance_strategy'] else None,
            quality_names=get_quality_names(exercise_code=exercise_code, df=features_df)
        )
    else:
        cv_results = perform_cross_validation_svm(
            X_train_reduced, y_train,
            n_splits=5, best_params=best_params,
            kernel=kernel if not best_params else best_params.get('kernel', kernel),
            class_weight='balanced' if imbalance_config['use_imbalance_strategy'] else None
...
if __name__ == "__main__":
    run_classification_pipeline_svm()
